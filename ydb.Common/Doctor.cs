    ...
    public class Doctor
    {
        ...
        public string GetList(string xmlString)
        {
            ...
            try
            {
                ...
                vNode = doc.SelectSingleNode("GetDoctorList/HospitalID");
                if (vNode != null)
                {
                    val = vNode.InnerText.Trim();
                    if (val.Length > 0 && val != "-1")
                        filter = filter.Length > 0 ? filter = filter + " And  t1.FHospitalID='" + val + "'" : " t1.FHospitalID='" + val + "'";
                }

                sql = "SELECT t1.*,t2.FName,t2.FNumber,t2.FFullNumber,t2.FClassID,t2.FLevel,t2.FDescription,t2.FLevel,t2.FIsDetail," +
                    " Isnull(t3.FName,'') As FHosptal,Isnull(t4.FName,'') As FDeptment,Isnull(t6.FName,'') As FRankName ,Isnull(t5.FName,'') As FTitleName " +
                    " From t_Doctors t1" +
                ...
                if (dt.Rows.Count > 0)
                {
                    result = "<?xml version=\"1.0\" encoding=\"utf-8\"?><GetDoctorList>" +
                            "<Result>True</Result>" +
                            "<Description></Description>" +
                            "<DataRows></DataRows>" +
                            "</GetDoctorList>";
                    doc.LoadXml(result);
                    XmlNode pNode = doc.SelectSingleNode("GetDoctorList/DataRows");
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        vNode = doc.CreateElement("FIDNumber");
                        vNode.InnerText = row["FIDNumber"].ToString();
                        cNode.AppendChild(vNode);
        ...
        public string Update(string dataString)
        {
            ...
            try
            {
                ...
                //更新消息信息
                vNode = doc.SelectSingleNode("UpdateItem/ID");
                if(vNode!=null)
                {
                    id = vNode.InnerText;
                    if (id.Length == 0 || id == "-1")
