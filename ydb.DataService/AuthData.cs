...
namespace ydb.DataService
{
    public class AuthData
    {
        ...
        public DataTable GetUploadDataFromOA()
        {
            ...
            sql = @"Insert Into [DataService].dbo.[OAAuthData](FInstitutionNumber,FInstitutionNanme,FObjectID,FObjectNumber,FObjectName,FAuthType,FTID,FInstitutionID)
                         Select   t1.field0001 FInstitutionNumber, t1.field0002 FInstitutionNanme ,t1.field0004 AS FObjectID, t2.CODE  As FObjectNumber,t2.NAME AS FObjectName, 
                        4 As FAuthType,(t1.field0001 +'-' + t2.CODE ) AS FTID,t3.ID AS FInstitutionID 
                        from  formmain_8739  t1 --t1 授权表
						 Left Join ORG_MEMBER t2 On t1.field0004=  Cast(t2.ID  AS  nvarchar(20))						  
                        Left Join formmain_8044 t3 On t1.field0001= t3.field0001 --t3 医院表
                        Where   Isnull(t1.field0004,'')<>'' and  Isnull(t1.field0001,'')<>'' 
						and  Isnull(t1.field0004,'')<>'' and (t1.field0001 +'-' + t2.CODE ) Not In(Select Distinct FTID from [DataService].dbo.[YDBAuthData]) 
";
            #region 授权人同步语句 union
            // Select   t1.field0001 FInstitutionNumber, t1.field0002 FInstitutionNanme ,t1.field0004 AS FObjectID, t2.CODE  As FObjectNumber,t2.NAME AS FObjectName, 
            //4 As FAuthType,(t1.field0001 + '-' + t2.CODE ) AS FTID, t3.ID AS FInstitutionID
            //                        from(select * from formmain_8739 t1
            //cross apply(SELECT * FROM dbo.SplitString(field0005, ',')) v where LEN(field0005) > 20)  t1--t1授权表
            //                     Left Join ORG_MEMBER t2 On t1.[Value] = Cast(t2.ID  AS  nvarchar(20))
            //                        Left Join formmain_8044 t3 On t1.field0001 = t3.field0001--t3医院表
            //                        Where   Isnull(t1.field0005, '') <> '' and Isnull(t1.field0001,'')<> ''

            //                        and Isnull(t1.field0005,'')<> '' and(t1.field0001 + '-' + t2.CODE) Not In(Select Distinct FTID from [DataService].dbo.[YDBAuthData])
            #endregion
            runner.ExecuteSqlNone(sql);
            sql = @" Select *,'0' As FUploadOption from [DataService].dbo.[OAAuthData] where Isnull(FInstitutionID, '') <> '' order by  FTID,FAuthType Desc";
